...
For the current 0.2.8 release:

```bash
git tag -a v0.2.8 -m "GraphVault TS 0.2.8"
git push origin v0.2.8
```

## Publishing

Use the GitHub Actions `Release` workflow with the matching tag input, for example `v0.2.8`, and set the confirmation input to `PUBLISH`.
